...
const int ltSonarAngle = 45;
const int rtSonarAngle = 45;
...
// 
  ...
  if(sensorArray[0] < 7){
  ...
  else if(sensorArray[2] < 7){
      spin(500);
      reverse(7);
    }
  else if(sensorArray[4] < 7){
      spin(-500);
      reverse(7);
    }
      ...
      while(sensorArray[5] < 2 || sensorArray[6] < 2){
          ...
          reverse(fvectormag);
          lSonarAvg = readLeftSonar();
          rSonarAvg = readRightSonar();
        }
    }
  else{
      stepperLeft.stop();
...
//  }
//  avg = avg/10;
  avg = analogRead(ftIRPin);
  float dist = 24.4*exp(-0.00948*avg) + 1.66;
  return dist;
}

float readBackIR() { 
  int avg= analogRead(bkIRPin);
//  for (int i = 0; i < 9 ; i++) {
...
float VectorFrontAngle(float fIRdist, float lSonardist, float rSonardist){
   ...
   float fvectorx = fIRdist + lSonardist*sin(ltSonarAngle) + rSonardist*sin(rtSonarAngle);
   float fvectory = lSonardist*cos(ltSonarAngle) - rSonardist*cos(rtSonarAngle);
